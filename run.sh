...
# start the FastAPI app, enabling hot reload on save (assuming deepcuts_api packages is installed)
function run {
     uvicorn deepcuts_api.main:create_app --reload --factory
}


# start the FastAPI app, enabling hot reload on save (assuming deepcuts_api packages is not installed)
function run-py {
    uvicorn deepcuts_api.main:create_app --reload --factory
}
...
function test:ci {
    INSTALLED_PKG_DIR="$(python -c 'import deepcuts_api; print(deepcuts_api.__path__[0])')"
